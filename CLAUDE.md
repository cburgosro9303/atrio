...
Hay **tres envolventes** y cada esquema lleva exactamente la suya, verificado por el test: la completa para los artefactos de proyecto, la reducida para el front-matter y `manifestEnvelope` para el manifiesto del marketplace, que no es artefacto de ningún proyecto sino un índice publicado por tag. La familia por defecto es la de artefacto, así que un esquema nuevo queda sujeto a esa regla hasta que alguien decida otra cosa. `common.schema.json` es el único sitio de cada concepto compartido; dos pares están sincronizados por test y no por comentario: las siete categorías de permisos, y `catalogId` como prefijo literal de `catalogRef`.

## Estado actual

**T-001, T-002, T-010 y T-011 completadas.** Módulo `github.com/cburgosro9303/atrio`, Go 1.26, los 8 paquetes con sus `doc.go`, test de arquitectura, pipeline de CI y los 10 esquemas —los 9 de artefactos más el manifiesto del marketplace— con su arnés de pruebas. **ADR-017** añadido (front-matter YAML validado, acota ADR-005). El catálogo de capacidades **no** está congelado en el esquema: `requiredCapabilities` lleva patrón y la pertenencia la verifica el core, para que la capacidad que aparezca en T-050 sea una release de plataforma y no un cambio incompatible de la API pública. Próxima tarea: **T-012** (definición canónica de agente y de flujo declarativo), tarea de diseño que consume las convenciones de T-010 y desbloquea T-050 y T-060. Riesgo prioritario a despejar temprano: spike de T-053 (arbitraje de permisos vía hooks de Claude Code) antes de congelar el diseño fino de T-050.
